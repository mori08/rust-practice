...
### 外部クレートへの委譲とエラー型の設計（walkdir）
- [x] `?`は暗黙に`From::from`を通す — `Err(e) => return Err(From::from(e))`と展開されるので、**戻り値のエラー型に変換できるなら別のエラー型もそのまま`?`できる**。`walkdir::Error`を返すイテレータを`io::Error`を返す関数で`?`できたのは、walkdirが`impl From<walkdir::Error> for io::Error`を提供しているから。既習の「エラー型は変換先が決める」の正体がこれ
- [x] **戻り値のエラー型は「実装が何を知っているか」でなく「呼び出し側が何を区別するか」で決める** — `walkdir::Error`のほうが情報は多い（深さ・リンク循環）が、呼び出し側が使わないなら対価だけ payしている。外部クレートのエラー型をシグネチャに出すと依存が漏れ、乗り換え時にシグネチャごと変わる。厳密にしたいときの正解は自作enum（`Box<dyn Error>`/anyhowとの使い分けは今後）。C++で言えばヘッダに実装の詳細な型を書かない指針と同じ
- [x] シグネチャを変えずに中身を入れ替えられた＝責務が切れている証拠。`diff`もテスト4本も無修正で通った
- [x] `walkdir::DirEntry`は`std::fs::DirEntry`と**同名の別型** — `path()`は`&Path`を返す（stdは`PathBuf`）。同名でもクレートが違えば別物、docs.rsで実物を確認する習慣
- [x] `to_` / `into_` の命名慣習 — `to_path_buf()`は借用から複製、`into_path()`は`self`を消費して中身をそのまま取り出す（複製なし）。標準ライブラリ全体で一貫
- [x] ムーブできるかは借用チェッカーが判断 — `entry.metadata()`は所有された`Metadata`を返すので`entry`を借りたままにしない。だから直後に`entry.into_path()`でムーブできる（`&Metadata`を返す設計ならE0382になっていた）
- [x] `file_type()`は走査時のキャッシュを読むだけ（システムコールなし・`Result`ですらない）、`metadata()`は毎回問い合わせる。ディレクトリ判定を`file_type()`で先に済ませると無駄な問い合わせが減る
- [x] 推移的依存に設計判断が現れる — `walkdir`が引き込む`same-file`はシンボリックリンクの循環検出用。「stdが再帰を持たない理由」が`Cargo.lock`から読み取れる
- [x] ビルダーパターン — `WalkDir::new(p).max_depth(n).follow_links(b)`。`new()`が即完成品を返さず設定を重ねる形（今回は既定値のままで足りたので未使用）
- [x] **マイルストーン: ウォッチャー段階7（walkdir化）完成**（`scan`から自前再帰が消え平坦なループに、Issue #4）
...
- [ ] **CLIツール制作（最終目標）: ファイル変更ウォッチャー**
  ...
  - [x] 段階7: `walkdir`への置き換え（Issue #4、クローズ済み） — 自前再帰を捨てて`WalkDir::new(path)`のフラットなイテレータに。`entry.file_type().is_dir()`で早期`continue`、`entry.into_path()`でコピーなしにキー化。`scan`のシグネチャは`io::Error`のまま変えず、`?`が`From`でエラー型を変換してくれることを確認した
    - 標準`read_dir`が1階層な理由 = 再帰は「シンボリックリンクを辿るか」「深さ制限」「エラー時に中断か継続か」など万人向けの既定値がなく、stdに入れると後方互換で直せなくなるため。stdは小さく、判断の分かれるものはエコシステムへ
  - [ ] **← 次回ここから**: 未起票の候補（着手を決めた時点でIssueを立てる）
    - `notify`（OSのファイル変更通知。ポーリング設計自体をやめる。mpscチャンネルが絡む）
    - エラー処理の作り込み（自作エラーenum / `anyhow`）。いま`scan`は`if let Ok(...)`でmetadata取得の失敗を黙って捨てており、walkdirが返す走査エラーも活かせていない
    - `WalkDir`のビルダー活用（`--max-depth`オプションをCLIに生やす、`follow_links`の扱いを決める）
